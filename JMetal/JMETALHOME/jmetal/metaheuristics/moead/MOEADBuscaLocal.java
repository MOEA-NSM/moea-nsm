...
public class MOEADBuscaLocal extends Algorithm {
     ...
     */
    public MOEADBuscaLocal(Problem problem) {
        super(problem);
    ...
    public SolutionSet execute() throws JMException, ClassNotFoundException {
        ...
        T_ = ((Integer) this.getInputParameter("T")).intValue();//quantidade de vizinhos de cada solucao  - Vizinhanca
        nr_ = ((Integer) this.getInputParameter("nr")).intValue(); //maximo numero de solucoes substituidas por cada solucao filha
        delta_ = ((Double) this.getInputParameter("delta")).doubleValue();//probabilidade de solucoes pais serem selecionadas de solucoes vizinhas
        ...
        buscaLocalOperator = operators_.get("buscaLocal");
        ...
        do {
            ...
            for (int i = 0; i < populationSize_; i++) {
                ...
                child = (Solution) buscaLocalOperator.execute(child);
                ...
                child2 = (Solution) buscaLocalOperator.execute(child2);

                // Apply DE crossover 
//        child = (Solution) crossover_.execute(new Object[]{population_.get(n), parents});
                // Apply mutation
                mutation_.execute(child);
                mutation_.execute(child2);//OBSERVAR, ANTES SO EXISTIA child1 

                // Evaluation
                problem_.evaluate(child);
                problem_.evaluate(child2);   //OBSERVAR, ANTES SO EXISTIA child1    
                ...
                updateReference(child2);//OBSERVAR, ANTES SO EXISTIA child1 

                // STEP 2.5. Update of solutions
                updateProblem(child, n, type);
                updateProblem(child2, n, type);//OBSERVAR, ANTES SO EXISTIA child1 

                if (indicators != null) {
                    double HVAtual = indicators.getHypervolume(population_);
                    ...
                    if (estagnacao >= MAX_ESTAGNACAO) {
                        System.out.println("MOEAD BL Estagnou na geracao: " + evaluations_);
                        break;
                    }
                } // if
        ...
        population_.setTempoExecucao(tempoExecucao.getTempo());
        population_.setGeracaoParou(evaluations_);
...
//            int objetivoSelecionado = UtilAP.obterNumeroAleatorio(0, 2);
//            newSolution.setObjetivoBuscaLocal(objetivoSelecionado);
//            newSolution.setInstancia((objetivoSelecionado == 0) ? problem_.instancia1 : problem_.instancia2);
//            newSolution = (Solution) buscaLocalOperator.execute(newSolution);
