...
public abstract class Experiment {
    ...
    public Experiment() {
        ...
        indicatorMinimize_.put("R2", true);
        ...
        if (paretoFrontDirectory_.equals("") || paretoFrontDirectory_ == null) {
            ...
            for (int i = 0; i < problemList_.length; i++) {
                if (paretoFrontFile_[i] == null || paretoFrontFile_[i].equals("")) {
                    ...
                    String filePath = paretoFrontDirectory_ + UtilAP.barra() + paretoFrontFile_[i];
                    File f = new File(filePath);
                    if (!f.exists()) {
                        paretoFrontFile_[i] = "";
                        generateReferenceFronts(i);
                    } else {
                        frontPath_[i] = paretoFrontDirectory_ + UtilAP.barra() + paretoFrontFile_[i];
                    } // if
                } // if
            } // for
        } // if
    ...
    public void generateQualityIndicators() {
        ...
        /*
         TODOS OS INDICADORES DE QUALIDADE SÃO BASEADO NAS FRONTEIRAS DE NÃO DOMINANCIA GERADA POR CADA ABORDAGEM
         */
        checkParetoFronts();
        ...
        if (indicatorList_.length > 0) {
            ...
            for (int algorithmIndex = 0; algorithmIndex < algorithmNameList_.length; algorithmIndex++) {
                ...
                algorithmDirectory = experimentBaseDirectory_ + UtilAP.barra() + "data" + UtilAP.barra() + algorithmNameList_[algorithmIndex] + UtilAP.barra();
                ...
                for (int problemIndex = 0; problemIndex < problemList_.length; problemIndex++) {
                    ...
                    for (String anIndicatorList_ : indicatorList_) {
                        ...
                        UtilAP.resetFile(problemDirectory + UtilAP.barra() + anIndicatorList_);

                        for (int numRun = 0; numRun < independentRuns_; numRun++) {

                            String outputParetoFrontFilePath;
                            outputParetoFrontFilePath = problemDirectory + UtilAP.barra() + "FUN." + numRun;
                            ...
                            if (anIndicatorList_.equals("HV")) {
                                ...
                                qualityIndicatorFile = qualityIndicatorFile + UtilAP.barra() + "HV";
                            ...
                            if (anIndicatorList_.equals("SPREAD")) {
                                ...
                                qualityIndicatorFile = qualityIndicatorFile + UtilAP.barra() + "SPREAD";
                            }
                            if (anIndicatorList_.equals("IGD")) {
                                ...
                                qualityIndicatorFile = qualityIndicatorFile + UtilAP.barra() + "IGD";
                            }
                            if (anIndicatorList_.equals("EPSILON")) {
                                ...
                                qualityIndicatorFile = qualityIndicatorFile + UtilAP.barra() + "EPSILON";
                            }
                            if (anIndicatorList_.equals("R2")) {
                                R2 indicators = new R2();
                                double[][] solutionFront = indicators.utils_.readFront(solutionFrontFile);
                                value = indicators.R2(solutionFront, trueFront);
                                qualityIndicatorFile = qualityIndicatorFile + UtilAP.barra() + "R2";
                            }
                    ...
                    /**
                     * CALCULA A MEDIA DE TEMPO E DE GERACOES
                     */
                    String pathTempo = problemDirectory + UtilAP.barra() + "TEMPO.txt";
                    String pathAvaliacao = problemDirectory + UtilAP.barra() + "AVALIACAO.txt";

                    UtilAP.salvarValorArquivo(pathTempo, "MEDIA=" + TempoExecucao.toString(true, UtilAP.calcularMediaGeracaoTempo(pathTempo)));
                    UtilAP.salvarValorArquivo(pathAvaliacao, "MEDIA=" + String.valueOf(UtilAP.calcularMediaGeracaoTempo(pathAvaliacao)));
    ...
    public void generateReferenceFronts(int problemIndex) {
        ...
        File rfDirectory;
        String referenceFrontDirectory = experimentBaseDirectory_ + UtilAP.barraDupla() + "referenceFronts";
        ...
        frontPath_[problemIndex] = referenceFrontDirectory + UtilAP.barra() + problemList_[problemIndex] + ".rf";
        ...
        for (String anAlgorithmNameList_ : algorithmNameList_) {
            ...
            String problemDirectory = experimentBaseDirectory_ + UtilAP.barra() + "data" + UtilAP.barra() + anAlgorithmNameList_
                    + UtilAP.barra() + problemList_[problemIndex];
    ...
    public void generateLatexTables() throws FileNotFoundException, IOException {
        latexDirectory_ = experimentBaseDirectory_ + UtilAP.barra() + latexDirectory_;
        ...
        for (int indicator = 0; indicator < indicatorList_.length; indicator++) {
            ...
            for (int problem = 0; problem < problemList_.length; problem++) {
                ...
                for (int algorithm = 0; algorithm < algorithmNameList_.length; algorithm++) {
                    ...
                    directory += UtilAP.barra() + "data" + UtilAP.barra();
                    directory += UtilAP.barra() + algorithmNameList_[algorithm];
                    directory += UtilAP.barra() + problemList_[problem];
                    directory += UtilAP.barra() + indicatorList_[indicator];
        ...
        //System.out.println("Experiment name: " + experimentName_);
        String latexFile = latexDirectory_ + UtilAP.barra() + experimentName_ + ".tex";
...
//    os.write("\\maketitle" + "\n");
    ...
    void printMeanStdDev(String fileName, int indicator, double[][][] mean, double[][][] stdDev) throws IOException {
        ...
        os.write("\\caption{" + indicatorList_[indicator] + ". Media e desvio padr\\~ao}" + "\n");
    ...
    void printMedianIQR(String fileName, int indicator, double[][][] median, double[][][] IQR) throws IOException {
        ...
        os.write("\\" + "\n");
        os.write("\\begin{table}" + "\n");
        os.write("\\caption{" + indicatorList_[indicator] + ". Media e IQR}" + "\n");
