...
public class SolutionSet implements Serializable {
    ...
    /**
     * Armazena a avaliação que o algoritmo parou, sendo por estagnacao ou por
     * limite definido
     *
     */
    private int geracaoParou;
    /**
     * Armazena o tempo de execucao de cada execução
     *
     */
...
//            Configuration.logger_.severe("\t Size is: " + this.size());
        ...
        if (!solutionsList_.contains(solution)) {
    ...
    public void printObjectivesToFileCSV(String path) {
        try {
            /* Open the file */
            FileOutputStream fos = new FileOutputStream(path);
            OutputStreamWriter osw = new OutputStreamWriter(fos);
            ...
            for (Solution aSolutionsList_ : solutionsList_) {
                //if (this.vector[i].getFitness()<1.0) {
                bw.write(aSolutionsList_.toString().replaceAll("\\s+", ";").replaceAll("\\.", ","));
                bw.newLine();
                //}
            }
    ...
    public void printFeasibleFUNCSV(String path) {
        try {
            FileOutputStream fos = new FileOutputStream(path);
            OutputStreamWriter osw = new OutputStreamWriter(fos);
            BufferedWriter bw = new BufferedWriter(osw);
    ...
    public SolutionSet union(SolutionSet solutionSet) {
        ...
        for (int i = this.size(); i < (this.size() + solutionSet.size()); i++) {
            /**
             * **If adicionado para nao adicionar solucoes repetidas***********
             */
//            if (!union.solutionsList_.contains(solutionSet.get(i - this.size()))) {
...
//            }
    ...
    public void printTempo(String path) {
        UtilAP.salvarValorArquivo(path, String.valueOf(this.tempoExecucao));
    }

    public void printGeracaoParou(String path) {
        UtilAP.salvarValorArquivo(path, String.valueOf(this.geracaoParou));
    }
    ...
    public int getGeracaoParou() {
        return geracaoParou;
    }

    public void setGeracaoParou(int geracaoParou) {
        this.geracaoParou = geracaoParou;
    }
